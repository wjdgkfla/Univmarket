...
const root = fileURLToPath(new URL(process.argv.includes('--student-qa') ? '../build/student-qa/' : '../build/web/', import.meta.url));
const port = process.argv.includes('--student-qa') ? 4174 : 4173;
...
}).listen(port, '127.0.0.1', () => console.log(`UnivMarket preview: http://127.0.0.1:${port}`));
