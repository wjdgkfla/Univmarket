-- Push notifications (FCM), following
-- https://supabase.com/docs/guides/functions/examples/push-notifications
--
-- Every chat event is a messages row with a recipient: texts, offers, and the
-- system notes for accepted/declined/withdrawn offers and finished
-- reservations. A messages trigger turns each into a notifications row; a
-- notifications trigger queues the push Edge Function through pg_net, which
-- sends only after the transaction commits.
...
exception when others then
  -- A notification must never block the message itself.
...
-- Needs 'project_url' in Vault (see configure_photo_cleanup.sql); without it
...
-- What the push function needs for one notification: its text, the
-- recipient's devices, and their unread count for the app badge.
...
-- A device token belongs to whoever signed in on it last, so a shared phone
-- never receives the previous student's messages.
...
-- Opening a chat reads its notifications, so the app badge counts down.
